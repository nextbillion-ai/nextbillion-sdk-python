...
class ResultRouteStep(BaseModel):
    ...
    **Note:** Since both integer and string value types are supported for job IDs,
    ...
    given task in the `jobs` or the `shipments` objects of the input POST
    ...
    Its value will always be one of the following: `start`, `job`, `pickup`,
    `delivery`, `end`. In case the type is `start` or `end`, `steps` object will not
    have the `id` field.
    """
...
class ResultRoute(BaseModel):
    ...
    It would be the same as that provided in the `description` field of `vehicles`
    part of the input POST optimization request.
    """
