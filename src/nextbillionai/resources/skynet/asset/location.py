...
class LocationResource(SyncAPIResource):
    ...
    def list(
        ...
        *,
        ...
        correction: str | Omit = omit,
        end_time: int | Omit = omit,
        geometry_type: Literal["polyline", "polyline6", "geojson"] | Omit = omit,
        pn: int | Omit = omit,
        ps: int | Omit = omit,
        ...
        return self._get(
            path_template("/skynet/asset/{id}/location/list", id=id),
    ...
    def get_last(
        ...
        *,
        ...
        if not id:
            raise ValueError(f"Expected a non-empty value for `id` but received {id!r}")
        return self._get(
            path_template("/skynet/asset/{id}/location/last", id=id),
            options=make_request_options(
                extra_headers=extra_headers,
                extra_query=extra_query,
                extra_body=extra_body,
